...
const chatbotController = {
  ...
  ask: async (req, res) => {
    ...
    try {
      ...
      const userKey = req.user.id || 'all';

      // 1. Get or Create Session (Synchronous & Immediate)
      let session;
      if (sessionId) {
      ...
      if (clearCommands.includes(cleanMsg)) {
        ...
        setImmediate(async () => {
          try {
            ...
            console.error("Async clear chat greeting save failed:", err);
          }
        });
      ...
      // 3. Process Intent
      console.time("chatbot-intent");
      let answer = "";
      let type = "unsupported";
      let source = "system";

      // A. Check built-in Dictionary (Instant local resolution)
      ...
      // 6. Return response payload instantly
      res.json({
      ...
      setImmediate(async () => {
        try {
          ...
          console.error("Async chatbot database history logging failed:", err);
        }
      });
